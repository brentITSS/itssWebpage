...
const ContactLogDetail: React.FC = () => {
  ...
  const loadContactLog = useCallback(async () => {
    ...
    try {
      ...
      const [logData, tagData] = await Promise.all([
        contactLogService.getContactLog(parseInt(id)),
        tagService.getTagLogsByEntity('ContactLog', parseInt(id)),
      ]);
      setContactLog(logData);
      setTags(tagData);
    } catch (err: any) {
      setError(err.message || 'Failed to load contact log');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTagAdded = async () => {
    if (!id) return;
    try {
      const tagData = await tagService.getTagLogsByEntity('ContactLog', parseInt(id));
      setTags(tagData);
    } catch (err: any) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await contactLogService.addAttachment(parseInt(id), file);
      loadContactLog(); // Reload to get updated attachments
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <dl className="grid grid-cols-1 gap-4">
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-gray-900">{contactLog.propertyName}</dd>
            </div>
            {contactLog.tenantName && (
              <div>
                <dt className="text-sm font-medium text-gray-500">Tenant</dt>
                <dd className="mt-1 text-sm text-gray-900">{contactLog.tenantName}</dd>
              </div>
            )}
            <div>
              <dt className="text-sm font-medium text-gray-500">Contact Log Type</dt>
              <dd className="mt-1 text-sm text-gray-900">{contactLog.contactLogTypeName}</dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-gray-500">Subject</dt>
              <dd className="mt-1 text-sm font-semibold text-gray-900">{contactLog.subject}</dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-gray-500">Notes</dt>
              <dd className="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{contactLog.notes}</dd>
            </div>
            <div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            {contactLog.attachments.length === 0 ? (
              <p className="text-sm text-gray-500">No attachments</p>
            ) : (
              <div className="space-y-2">
                {contactLog.attachments.map((attachment) => (
                  <div
...
export default ContactLogDetail;
