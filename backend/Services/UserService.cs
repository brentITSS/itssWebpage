...
public class UserService : IUserService
{
    ...
    public async Task<ResetPasswordResponse?> ResetPasswordAsync(int userId, ResetPasswordRequest request, int modifiedByUserId)
    {
        var user = await _userRepository.GetByIdForUpdateAsync(userId);
        if (user == null) return null;

        var plainPassword = request.GenerateTemporaryPassword
        ...
        try
        {
            await _auditLogRepository.CreateAsync(new AuditLog
            {
                ...
                EntityType = "User",
                EntityId = userId,
                NewValues = request.GenerateTemporaryPassword
                    ? "Temporary password generated by Global Admin."
                    : "Password reset by Global Admin.",
                CreatedDate = DateTime.UtcNow
            });
        }
        catch (Exception ex)
        ...
        {
            Message = request.GenerateTemporaryPassword
                ? "Temporary password generated. User must change password on next login."
                : "Password reset successfully.",
            TemporaryPassword = request.GenerateTemporaryPassword ? plainPassword : null
        };
    }
